...
class Conv2D:
    ...
    def initialize_weights(self):
        ...
        if self.initialize_method == "random":
            ...
            f_in = self.kernel_size[0] * self.kernel_size[1] * self.in_channels
            f_out = self.kernel_size[0] * self.kernel_size[1] * self.out_channels
            scale = np.sqrt(2 / float(f_in + f_out))
            ...
            for h in range(n_H):
                ...
                for w in range(n_W):
                    ...
                    for c in range(C):
                        ...
                        weights = W[:, :, :, c]
                        biases = b[:, :, :, c]
                        Z[i, h, w, c] = self.single_step_convolve(a_slice_prev, weights,
                                                                  biases)
        return Z
